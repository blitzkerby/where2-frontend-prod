    ...
    <>
      <nav className="bg-gray-100 h-[64px] w-full fixed top-0 left-0 right-0 z-[1002]">
        <div className="flex justify-between gap-x-10 items-center px-4 py-3 h-full sm:hidden m-auto lg:w-7/12 sm:hidden">
          ...
          <div className="flex align-center justify-between space-x-20 sm:hidden h-full lg:w-[60%]">
            {menuItems.map((item) => (
              <Link key={item.name} to={item.to} className="flex my-[6px] text-gray-700 hover:text-gray-900 h-full">
                {item.name}
              </Link>
            ))}
          </div>
